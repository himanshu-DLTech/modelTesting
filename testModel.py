...
for task in tasks:
    ...
    try:
        ...
        results.append(f"Task: {task['name']}\n❌ Failed to get response from Ollama: {e}\n")
        continue
    ...
    if result["success"]:
        ...
        for keyword in task["expected"]:
            if keyword.lower() in out or keyword.lower() in code.lower():
                ...
                failure_reason += f"Missing keyword: {keyword}\n"
        if score == len(task["expected"]):
            passed_cases += 1
    else:
        failure_reason = f"Runtime Error:\n{result['error']}"

    total_score += score
    max_score += len(task["expected"])

    # Log result for this task
    results.append(
        f"Task: {task['name']}\n"
        f"Score: {score}/{len(task['expected'])}\n"
        f"Status: {'✅ Passed' if score == len(task['expected']) else '❌ Failed'}\n"
        f"Reason: {failure_reason if failure_reason else 'All checks passed'}\n"
        f"{'-'*60}\n"
    )
...
print(summary)
print("📂 Results with per-task details saved to results.txt")
